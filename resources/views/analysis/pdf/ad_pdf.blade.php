...
if ($bool) {
    ...
    foreach ($ga_result_ad[1] as $key => $val) {
        $arr_click[] == 0 ? $arr_click[] = 1 : $arr_click[] = $val[0][0][1];
        $arr_cost[] == 0 ? $arr_cost[] = 1 : $arr_cost[] = $val[0][0][2];
        $arr_price[] == 0 ? $arr_price[] = 1 : $arr_price[] = $val[0][0][3];
        $arr_cv[] == 0 ? $arr_cv[] = 1 : $arr_cv[] = $val[0][0][4];
        $arr_cv_r[] == 0 ? $arr_cv_r[] = 1 : $arr_cv_r[] = $val[0][0][5];
    }
    rsort($arr_click);
    rsort($arr_cost);
    rsort($arr_price);
